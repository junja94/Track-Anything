...
def sam_refine(video_state, point_prompt, click_state, interactive_state, evt:gr.SelectData):
    ...
    # <Change2> Check new masking frame and 
    if "new_mask_start_frame" in interactive_state :  
        interactive_state["new_masks"].append((video_state["select_frame_number"], mask))

    # <Change> Inverse mask
    non_clicked_mask = np.logical_not(mask) 
          
    video_state["non_clicked_masks"][video_state["select_frame_number"]] = non_clicked_mask
...
def vos_tracking_video(video_state, interactive_state, mask_dropdown, save_dir):
    ...
    # <Change>
    # Generate non-clicked masks for all frames
    #non_clicked_masks = [np.logical_not(mask) for mask in masks]
    ...
    if last_valid_frame_index is None:
        last_valid_frame_index = len(masks)-1
    print(last_valid_frame_index)
    
    frame_number = video_state["select_frame_number"] + last_valid_frame_index
    video_state["masks"][video_state["select_frame_number"]:frame_number+1] = masks[:last_valid_frame_index+1]
    ...
    video_state["non_clicked_masks"] = dict(enumerate(non_clicked_masks[:last_valid_frame_index+1], start=video_state["select_frame_number"]))
    ...
    empty_frame_number = int(frame_number) if last_valid_frame_index < len(non_clicked_masks)-1 else None
    visible = last_valid_frame_index < len(non_clicked_masks)-1
    
    print(f"Empty frame number: {empty_frame_number}")
    print(f"Visible: {visible}")
    
    return video_output, video_state, interactive_state, operation_log, gr.update(value=empty_frame_number, visible= visible)



def save_frames_and_masks(video_state, masks, output_path):
    with h5py.File(output_path, 'w') as hf:
        # mask 데이터 저장
        all_masks = np.stack(masks, axis=0)
        n_frames = len(all_masks)
        
        mask_shape = masks[0].shape
        
        # 'mask' 데이터셋 생성
        mask_dataset = hf.create_dataset("mask", shape=(n_frames, mask_shape[0], mask_shape[1]), dtype='uint8', compression="gzip")
        
        for i, mask in enumerate(all_masks):
            frame_number = video_state["select_frame_number"] + i
            mask_dataset[i] = mask
            
            if np.all(mask == 255):  
                print(f"Empty frame detected at frame {frame_number}. Please add new mask.")
                break
        
        # 마지막 처리된 프레임 번호 저장
        hf.attrs['last_processed_frame'] = video_state["select_frame_number"] + n_frames - 1
...
if __name__ == "__main__":
    ...
    parser.add_argument("--port", type=int, default=6080, help="The port number for the server")
    ...
    parser.add_argument("--save_dir", type=str, default="logs", help="The directory to save the output")
    ...
    with gr.Blocks() as iface:
        ...
        gr.Examples(
            examples=[os.path.join(os.path.dirname(__file__), "./test_sample/", test_sample) for test_sample in ["test2.mp4","test-sample4.mp4", \
